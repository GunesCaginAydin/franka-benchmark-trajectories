...
int main(int argc, char** argv) {
    for (int i = 0; i < argv[2]; ++i) {
        std::cout << "Iteration: " << argv[i] << std::endl;
        ...
        double KPR_low = 5;
        kp.setZero();
        kp.topLeftCorner(3,3) = (Eigen::MatrixXd::Random(3,3) + Eigen::MatrixXd::Constant(3,3,1.))*(KP_high - KP_low) + Eigen::MatrixXd::Constant(3,3,KP_low);
        kp.bottomRightCorner(3,3) = (Eigen::MatrixXd::Random(3,3) + Eigen::MatrixXd::Constant(3,3,1.))*(KPR_high - KPR_low) + Eigen::MatrixXd::Constant(3,3,KPR_low);
        ...
        double KDR_low = 2;
        kd.setZero();
        kd.topLeftCorner(3,3) = (Eigen::MatrixXd::Random(3,3) + Eigen::MatrixXd::Constant(3,3,1.))*(KD_high - KD_low) + Eigen::MatrixXd::Constant(3,3,KD_low);
        kd.bottomRightCorner(3,3) = (Eigen::MatrixXd::Random(3,3) + Eigen::MatrixXd::Constant(3,3,1.))*(KDR_high - KDR_low) + Eigen::MatrixXd::Constant(3,3,KDR_low);
        ...
        double KIR_low = 1;
        ki.setZero();
        ki.topLeftCorner(3,3) = (Eigen::MatrixXd::Random(3,3) + Eigen::MatrixXd::Constant(3,3,1.))*(KI_high - KI_low) + Eigen::MatrixXd::Constant(3,3,KI_low);
        ki.bottomRightCorner(3,3) = (Eigen::MatrixXd::Random(3,3) + Eigen::MatrixXd::Constant(3,3,1.))*(KIR_high - KIR_low) + Eigen::MatrixXd::Constant(3,3,KIR_low);

        std::cout
        << "The randomized controller parameters are:\n"
        << "Proportional Gain: " << translational_stiffness << "\n"
        << "Derivative Gain: " << rotational_stiffness << "\n"
        << "Integral Gain: " << ki << "\n"
        << "kp Matrix:\n" << kp << "\n"
        << "kd Matrix:\n" << kd << "\n"
        << "ki Matrix:\n" << ki <<
